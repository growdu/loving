import type { ICardPlugin, CardVersion, Card } from '@/plugins/core/types'
...
const nanpuTasks = [
  ...
  '男仆JJ上涂黄酒用舌尖体验', '男仆裸体为女王按摩两分钟'
]

// All card versions
const allVersions: CardVersion[] = [
  { id: 'lover0', name: '恋爱版', category: 'lover', locked: false, tasks: lover0Tasks },
  { id: 'lover1', name: '热恋版', category: 'lover', locked: false, tasks: lover1Tasks },
  { id: 'sex0', name: '同居版', category: 'sex', locked: true, tasks: sex0Tasks },
  { id: 'sex1', name: '进阶版', category: 'sex', locked: true, tasks: sex1Tasks },
  { id: 'sex2', name: '私密版', category: 'sex', locked: true, tasks: sex2Tasks },
  { id: 'sm', name: 'SM版', category: 'sm', locked: true, tasks: smTasks },
  { id: 'huwai', name: '户外版', category: 'huwai', locked: true, tasks: huwaiTasks },
  { id: 'nvpu', name: '女仆版', category: 'nvpu', locked: true, tasks: nvpuTasks },
  { id: 'nanpu', name: '男仆版', category: 'nanpu', locked: true, tasks: nanpuTasks }
]

// Core Cards Plugin
export const coreCardsPlugin: ICardPlugin = {
  id: 'core-cards',
  name: '核心卡牌',
  version: '1.0.0',
  dependencies: [],
  ...
  getRandomCard(versionId: string) {
    const version = this.getVersion(versionId)
    if (!version) return undefined
    const index = Math.floor(Math.random() * version.tasks.length)
    return {
      id: `${versionId}-${index}`,
      versionId,
      index,
      task: version.tasks[index]
    }
  },

  getCard(versionId: string, index: number) {
    const version = this.getVersion(versionId)
    if (!version) return undefined
    const task = version.tasks[index % version.tasks.length]
    return {
      id: `${versionId}-${index}`,
      versionId,
      index: index % version.tasks.length,
      task
    }
  },

  isVipRequired(versionId: string) {
    const version = this.getVersion(versionId)
    return version?.locked ?? false
  }
}